...
int main(){
    ...
    //print out the time for finding the blocks
    clock_t end = clock();
    double time_spent = (double)(end - begin)/CLOCKS_PER_SEC;
    printf("Time spent to find blcoks %f\n",time_spent);
    ...
    // BubbleSortsign(id,indexs,signs);

    // quicksort for openmp here
    // quicksort(signs,0,indexs-1);
 

    //Print time used for sorting signtures
    clock_t end1 = clock();
    double time_spent1 = (double)(end1 - end)/CLOCKS_PER_SEC;
  
    printf("Time spent to sort the signtures: %f\n",time_spent1);
...
//Bubble sort method to sort col
...
int partition(long arr[], int low_index, int high_index)
{
int i, j, temp, key;
key = arr[low_index];
i= low_index + 1;
...
//quick sort with openmp to sort signtures
void quicksort(long arr[], int low_index, int high_index)
{
int j;
...
// Use openmp here to speed up the sort, here is the key point
